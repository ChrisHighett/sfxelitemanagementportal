...
UPDATE public.user_invites ui
   SET agency_id = pu.agency_id
  FROM public.portal_users pu
 WHERE ui.invited_by = pu.id
   AND ui.agency_id IS NULL;
...
BEGIN
  ...
  -- Prefer the agency stored on the invite (chosen at invite time, possibly by Eleva Ops).
  -- Fall back to the inviter's agency for older invites that pre-date this column.
  v_agency := v_invite.agency_id;
...
BEGIN
  ...
  VALUES (_new_user_id, v_invite.role, true, _display_name, v_invite.email, v_agency)
