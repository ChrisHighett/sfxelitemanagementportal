...
-- Allow divisional_gm to read scout_leads where the assigned agent is in their own division
CREATE POLICY "divisional_gm read scout_leads in division"
...
USING (
  ...
  AND EXISTS (
    ...
    WHERE pu.id = scout_leads.assigned_agent_id
      AND pu.agency_id = current_agency_id()
      AND pu.division_id = current_division_id()
  )
);

-- Update the three coaching RPCs to allow divisional_gm and scope to their division
...
BEGIN
  IF NOT (public.is_admin() OR v_is_eleva OR v_is_gm) THEN
  ...
  athlete_counts AS (
    ...
    WHERE a.assigned_agent_user_id IS NOT NULL
      ...
      AND (NOT v_is_gm OR a.agency_id = v_agency)
